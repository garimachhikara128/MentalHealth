...
for each_user_d,each_user_a in zip(users_d[0:-1], users_a[0:-1]) :
    ...
    for each_tweet in tweets[0:-1]:
        ...
        if  dep_dict[id] == '1' :
            ...
            sl[year-2017].append(repeatability(text))
            # nl[year-2017] += text + "."
...
plt.ylabel("Repeatability", fontsize=size) ### change
plt.xticks(fontsize=size)
plt.yticks(fontsize=size)
plt.legend(loc = "upper left" , fontsize= size)
### change
plt.savefig("/Users/garima/Desktop/Self/PhD/Research Work/Mental Health/Phase 2/13_DataTabular/plot_repeatability_sh_mean.pdf", format="pdf", bbox_inches="tight")
plt.show()