...
for each_user_d,each_user_a in zip(users_d[0:-1], users_a[0:-1]) :
    ...
    sl = [[],[],[],[],[],[]] ## to store redability for each year for a given user 
    ...
    for each_tweet in tweets[0:-1]:
        ...
        if dep_dict[id] == '1' :
            ...
            sl[year-2017].append(textstat.coleman_liau_index(text))
            # nl[year-2017] += text 
    ...
    for y in range(0,6) :
        ...
        # if(len(nl[y]) > 0) :
        #     res_user = round(textstat.coleman_liau_index(nl[y]),2)
        #     ll[y][idx].append(res_user)
...
plt.ylabel("Readability", fontsize=size) ### change
plt.xticks(fontsize=size)
plt.yticks(fontsize=size)
plt.legend(loc = "lower right" , fontsize= size)
### change
plt.savefig("/Users/garima/Desktop/Self/PhD/Research Work/Mental Health/Phase 2/13_DataTabular/plot_readability_sh_mean.pdf", format="pdf", bbox_inches="tight")
plt.show()